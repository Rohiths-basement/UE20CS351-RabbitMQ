...
c = mydb.cursor()
def create_table():
    c.execute('CREATE TABLE STUDENTS (name VARCHAR(50), SRN VARCHAR(15), section VARCHAR(1));')
# create_table()

app = Flask(__name__)

credentials = pika.PlainCredentials('Saanchita', 'Skanda&2008')
connection = pika.BlockingConnection(pika.ConnectionParameters('192.168.1.22', 5672, '/', credentials))
